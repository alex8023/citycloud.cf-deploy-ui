...
func (p execProcess) wait() Result {
	...
	logger.Debug("Successful: %t (%d)", err == nil, exitStatus)

	if err != nil {
		cmdString := strings.Join(p.cmd.Args, " ")
		err = NewExecError(cmdString, stdout, stderr)
	}
...
func (p *execProcess) TerminateNicely(killGracePeriod time.Duration) error {
	...
	if err != nil {
		return fmt.Errorf("Sending SIGTERM to process group %d", p.pid)
	}
	...
	case <-time.After(killGracePeriod):
		...
		if err != nil {
			return fmt.Errorf("Sending SIGKILL to process group %d", p.pid)
		}
	}
	...
	return fmt.Errorf("Failed to kill process after grace timeout (PID %d)", p.pid)
}
...
func (r execCmdRunner) RunComplexCommand(cmd Command) (string, string, int, error) {
	process := newExecProcess(r.buildComplexCommand(cmd))
...
func (r execCmdRunner) RunCommand(cmdName string, args ...string) (string, string, int, error) {
	process := newExecProcess(exec.Command(cmdName, args...))
